...
前序调研和 0715 复现实验已经暴露出一个明确问题：高缺失率下的预测困难不只是“缺几个值”，而是历史窗口中的可用时空上下文被系统性破坏。尤其在 Block-T 和 Block-ST 场景中，连续时间段内某个传感器、甚至一片空间邻域都会同时不可见，此时只依赖同一时刻的跨变量传播或简单前向填充，很容易失效。

HD-TTS 证明了一个重要方向：当局部时间或局部空间信息缺失严重时，模型应当转向更粗尺度的时间/空间表示，通过多尺度时空下采样扩大可用上下文。但 HD-TTS 本身不是插补模型，它主要通过多尺度表示直接服务预测，对“缺失区域内部的结构恢复”建模相对轻。CoFILL 和 P-GUTS 正好提供了另一类能力：它们虽然主任务是插补，但都在高维时空图数据和块缺失条件下专门优化了“从残缺时空结构中恢复合理信号”的能力。

因此，本轮实验的核心动机是验证一个研究假设：

> **插补模型中针对块缺失设计的时空结构恢复机制，如果不作为预测前的独立预处理，而是改造成端到端预测模型，可能提升高维高缺失率条件下的 forecasting；进一步叠加 HD-TTS 的多尺度时空下采样，有机会同时获得“恢复缺失结构”和“预测未来趋势”的能力。**

这也是本课题和传统两阶段“先插补再预测”的区别：本轮不是把 CoFILL/P-GUTS 当黑盒 imputer，而是把它们的结构恢复模块迁移进预测模型，让预测损失直接约束这些模块学到对未来有用的表示。
...
**第一，预测可以形式化为特殊插补。**  
P-GUTS 论文已经明确指出 forecasting 可以看成 future values imputation：把未来窗口拼接到历史窗口后，将未来位置的 mask 设为 0，模型输出这些未来缺失点即可。这样 P-GUTS 的 product graph U-Net 和 layer-wise denoising/refinement 机制可以几乎无缝迁移到预测任务。关键改动不是模型主体，而是数据构造、未来窗口 mask、预测损失和评估指标。

**第二，历史缺失恢复和未来预测共享时空依赖。**  
在 METR-LA、PEMS-BAY 这类交通图数据中，传感器之间存在稳定的空间相关，交通速度也有明显的短期传播和周期模式。插补任务学习的是“缺失位置如何从邻居、历史和周期中恢复”，预测任务学习的是“未来位置如何从历史、邻居和周期中推断”。两者依赖的结构信息高度重合，因此 CoFILL 的 TCN+GCN+DCT 条件特征、P-GUTS 的 product graph attention、多尺度 pooling 都有迁移基础。

**第三，HD-TTS 可以补足插补模型的尺度选择问题。**  
P-GUTS 的实验显示，多 pooling factor 在一般块缺失下有效，但在极高缺失率下不一定总优于单尺度；这说明不同缺失模式需要不同尺度，而固定拼接所有尺度可能引入冗余甚至噪声。HD-TTS 的 adaptive decoder 提供了可行解法：根据输入表示和 mask pattern 动态选择时间/空间尺度。因此 HD-PGUTS 的可行性在于，用 P-GUTS 负责 product graph 上的局部-全局恢复，用 HD-TTS 式尺度权重负责在高缺失模式下选择可靠上下文。

**第四，代码和数据具备落地条件。**  
CoFILL、P-GUTS、HD-TTS、BiTGraph 都有可访问代码；METR-LA 和 PEMS-BAY 已经下载到 `dataset/`，且是这些论文共同使用的高维交通图数据。P-GUTS 附录已经提供 imputation 和 forecasting 命令，CoFILL 官方代码可先复现 block imputation，HD-TTS 已在前序计划中验证过可作为多尺度预测 baseline。也就是说，本计划不是从零实现整套模型，而是在已有代码基础上做任务接口和模块融合，工程风险可控。
