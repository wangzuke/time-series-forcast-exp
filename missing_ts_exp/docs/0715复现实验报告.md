...
- `missing_ts_exp/results/0715_bitgraph_hdtts/csv/unified_high_missing.csv`（Phase 3 统一协议，32 行；其中 BiTGraph 点缺失 60%/80% 两个数据点复用自 Phase 2 忠实复现结果，见 3.1 节说明）
...
|---|---|---|---|---|---|
| BiTGraph/PEMS-BAY | 1.99 | 2.03 | 2.08 | 2.15 | +8.0% |
| BiTGraph/METR-LA | 6.20 | 6.44 | 6.44 | 6.55 | +5.6% |
| HD-TTS/PEMS-BAY | 1.65 | 1.67 | 1.71 | 1.80 | +9.1% |
| HD-TTS/METR-LA | 3.16 | 3.21 | 3.26 | 3.34 | +5.7% |

**数据来源说明**：BiTGraph 60%/80% 两个数据点来自 Phase 2 忠实复现（官方超参、`seq_len=24/pred_len=24`、`random_point`，见 2.1 节表格），50%/70% 两个数据点来自 Phase 3 统一协议扩展（`mask_seed=42`）；两批数据分属不同 run（种子不同：Phase 2 未记录/随机种子，Phase 3 固定 `mask_seed=42`），拼成同一条曲线用于覆盖 50-80% 全区间。HD-TTS 的 4 个数据点均来自 Phase 3 同一批次运行（单一随机种子）。因此曲线内部存在"半段用一个种子、半段用另一个种子"的不一致性，仍应理解为单次运行的点估计，而非跨种子稳定的均值。

两个模型在点缺失下都表现出温和、非灾难性的退化，且现在两条曲线的采样点数一致（各 4 个），退化幅度也十分接近：BiTGraph（METR-LA +5.6%、PEMS-BAY +8.0%）与 HD-TTS（METR-LA +5.7%、PEMS-BAY +9.1%）几乎同一量级，进一步支持"点缺失场景下两者在各自协议内旗鼓相当、未见明显架构性优劣"的判断。
...
**倾向 HD-TTS，但优势微弱，主要基于训练成本而非 MAE 绝对值或退化幅度差距。** 原因：

1. 两个模型现在都覆盖了 50-80% 全部四个采样点（BiTGraph 60%/80% 数据点来自 Phase 2 忠实复现，见 3.1 节数据来源说明），退化曲线均完整，可以直接比较 50→80% 的相对变化。
2. 两者退化幅度非常接近（BiTGraph：METR-LA +5.6%、PEMS-BAY +8.0%；HD-TTS：METR-LA +5.7%、PEMS-BAY +9.1%），点缺失场景下没有出现明显的架构性劣势——这与块缺失场景（见 6.4）形成鲜明对比。
3. 训练成本上 HD-TTS 有早停机制、实际训练 epoch 数更少（61-158 epoch vs BiTGraph 固定 200 epoch），在 METR-LA 上单 epoch 训练时间也明显更短（~18s vs ~31s）。

**但不能忽视**：由于预测步长不同（HD-TTS horizon=12 vs BiTGraph horizon=24），HD-TTS 完成的是更简单的任务，这个"效率优势"某种程度上是在不对等的任务难度下取得的；同时 BiTGraph 曲线的 50/70% 与 60/80% 两段来自不同 run（不同种子），退化幅度的比较仍应视为方向性证据而非严格控制变量下的结论。
...
因此后续方法设计的合理方向是：以 HD-TTS 式的多尺度时空下采样作为主干，为跨时间的鲁棒性打底（尤其应对块/突发缺失），再叠加 BiTGraph 式 mask-aware 的跨变量图卷积模块，在同一时刻存在部分观测变量时显式利用这些变量做补偿——这与实验计划 §11 中"二者各有优势"对应的预期结论一致。点缺失下 BiTGraph 60%/80% 的数据缺口已在本次修订中用 Phase 2 忠实复现结果补齐（见 3.1 节），但仍是跨 run（不同种子）拼接的单点估计；若要进一步提高证据强度，建议后续用同一 `mask_seed` 协议重跑 BiTGraph 点缺失 50-80% 全区间，并补充多 seed 方差评估。
...
| 文件 | 行数/张数 | 说明 |
|---|---|---|
| `csv/unified_high_missing.csv` | 32 行（16 BiTGraph + 16 HD-TTS） | Phase 3 统一协议结果；BiTGraph 点缺失 60%/80% 两行复用自 Phase 2 忠实复现（见 3.1 节） |
