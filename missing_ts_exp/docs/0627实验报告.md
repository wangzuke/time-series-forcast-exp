...
| 实验组 | 命令数 | 完成数 | 核心问题 |
|---|---:|---:|---|
| A 主对比 | 144 | 137 | 原仓库对齐缺失感知方法是否胜过两阶段？ |
| B 高缺失率 | 64 | 59 | 极端缺失（50%/70%）下谁更鲁棒？ |
| C SAITS 溢出补跑 | 48 | 48 | 补全第一轮 SAITS+PatchTST 溢出空白 |
| D 掩码消融 | 24 | 21 | 掩码对原仓库对齐方法是否有效？ |
| E 扩展场景 | 64 | 56 | 原仓库对齐版本在变量通道缺失/混合缺失下表现 |
| F 误差传播 | 12 | 12 | 原仓库对齐方法的补值-预测误差关系 |
| **合计** | **356** | **333** | |
...
- **Weather（21 维）**：所有方法均可正常运行，CoIFNet(含 TSBlock/AttentionBlock/RevON) 在高缺失率下表现最优
- **Electricity（321 维）**：MissTSM(含 skip connection) 在此规模下反超两阶段方法；CRIB(原仓库对齐) 在 h336 出现优化不稳定（MSE 异常）
- **Traffic（862 维）**：CRIB(原仓库对齐) 因显存和训练时间代价过高全部跳过（batch_size=4，504 秒/epoch）；两阶段方法在此规模下优势最明显

#### 数据完整性

333/356 条实验产出有效结果。缺失的 23 条全部为 **Traffic（862通道）+ CRIB(原仓库对齐)** 组合。原因：CRIB 的双视图 patching + MultivariateNormal 后验在 862 通道下显存占用极大（batch_size=4 时单卡 ~72GB），且每 epoch 训练时间达 504 秒（约 8.4 分钟），单条实验约 3.5 小时。23 条全部跑完需额外 40+ 小时，性价比极低，决定跳过。该组合在本报告中不纳入对比分析。
...
| 变更项 | 第一轮 | 第二轮 | 理由 |
|---|---|---|---|
| 模型实现 | 去除模块变体 CRIB/CoIFNet/MissTSM | 原仓库对齐版本 | 排除模块差异对结论的影响 |
| 训练轮数 | 10 epoch, patience=3 | 20 epoch, patience=5 | 联合建模方法可能需更充分训练 |
| 缺失率 | 0.1, 0.3 | 新增 0.5, 0.7 | 验证高缺失率下方法反转 |
| 随机种子 | 3 个 | 2 个 | 第一轮种子间标准差很小 |

### 1.4 模型实现变更：从去除模块变体到原仓库对齐版本

第一轮实验中三个缺失感知方法均为相对原论文/GitHub 模型去除了部分模块的变体，本轮采用原始论文仓库中的原仓库对齐版本。为避免版本命名混淆，本文后续默认采用如下命名：

- R1 `CRIB(去除 patching/TCN/MultivariateNormal/双视图一致性)`
- R1 `CoIFNet(去除 TSBlock/AttentionBlock/时间特征，使用双 FFN MixerBlock)`
- R1 `MissTSM(去除 skip connection)`
- R2 `CRIB(原仓库对齐)`
- R2 `CoIFNet(含 TSBlock/AttentionBlock/RevON)`
- R2 `MissTSM(含 skip connection)`

以下逐一说明第一轮变体去除了哪些模块、第二轮原仓库对齐版本补齐了什么。

#### CRIB（模块差异：重 → 原仓库对齐变化最大）

| 特性 | 第一轮去除模块变体 | 第二轮原仓库对齐版本 |
|---|---|---|
| 输入格式 | 直接输入 `(B, L, C)`，不做 patching | 先分 patch 再编码 `(B, P, N, patch_len)`，patch_len=12 |
| 时序嵌入 | 简单 Linear | TCNBlock（指数膨胀因果卷积，dilation=2^i） |
| 后验分布 | 轻量重参数化 `mu + exp(0.5*logvar)*eps` | `MultivariateNormal(loc, diag(softplus(scale)))` |
...
原仓库对齐版本补齐了 CRIB 论文的全部核心贡献。但实验发现原仓库对齐版本在 Electricity 上反而退化 39%，高维数据集上 MultivariateNormal 后验可能优化不稳定。

#### CoIFNet（模块差异：中 → 原仓库对齐变化适中）
...
原仓库对齐版本在 Electricity 上改善 5%，Weather/Traffic 上基本持平（-1.2% / +0.4%），说明去除模块变体的联合框架已经捕获了主要信息。

#### MissTSM（模块差异：轻 → 原仓库对齐变化最小但效果最显著）
...
## 二、组 A：主对比——原仓库对齐缺失感知方法 vs 两阶段基准

### 2.1 核心结果

将原仓库对齐版本 MissTSM/CRIB/CoIFNet 与第一轮两阶段最优基准（Interp+iTrans 和 SAITS+iTrans）对比。两阶段基准直接复用第一轮数据。

**可视化总览**

![主对比：原仓库对齐版本 vs 去除模块变体](../figures_r2/fig1_main_comparison.png)

![R1 vs R2 对比](../figures_r2/fig3_r1_vs_r2.png)

#### Weather

| 缺失类型 | 缺失率 | 预测长度 | Interp+iTrans (R1) | SAITS+iTrans (R1) | MissTSM(含 skip connection) | CRIB(原仓库对齐) | CoIFNet(含 TSBlock/AttentionBlock/RevON) |
|---|---:|---:|---:|---:|---:|---:|---:|
...
| 缺失类型 | 缺失率 | 预测长度 | Interp+iTrans (R1) | SAITS+iTrans (R1) | MissTSM(含 skip connection) | CoIFNet(含 TSBlock/AttentionBlock/RevON) |
|---|---:|---:|---:|---:|---:|---:|
...
> **注**：Traffic+CRIB(原仓库对齐) 组合因显存和训练时间代价过高（batch_size=4，504 秒/epoch），已全部跳过，不纳入对比。

### 2.2 去除模块变体 vs 原仓库对齐版本对比

![改善幅度热力图](../figures_r2/fig9_improvement_heatmap.png)

| 方法 | 数据集 | R1 去除模块变体 MSE | R2 原仓库对齐版本 MSE | 改善幅度 |
|---|---|---:|---:|---:|
...
1. **MissTSM(含 skip connection) 在 Electricity 上取得全局最优**：MissTSM(含 skip connection) 在 Electricity 的多数配置中超越了两阶段基准（如随机点缺失 30%/h96: 0.1520 vs Interp+iTrans 0.1636，改善 7.1%），原仓库对齐版本 skip connection 的引入确实有效。在 Traffic 上也改善了 18.3%，但仍大幅落后于两阶段方法。

2. **CRIB(原仓库对齐) 出现严重退化**：原仓库对齐版本 CRIB 不仅没有改善，反而在 Electricity 上大幅退化（-39%）。Electricity h336 的 MSE 达到 0.53（R1 为 0.19）。原因可能是：原仓库对齐版本引入的 MultivariateNormal 后验和双视图一致性损失在高维场景下优化困难，20 epoch 仍不够收敛。Traffic+CRIB(原仓库对齐) 因计算代价过高已全部跳过（详见 1.2 节）。

3. **CoIFNet(含 TSBlock/AttentionBlock/RevON) 变化不大**：原仓库对齐版本在 Weather 上基本持平（-1.2%），在 Electricity 上小幅改善（+5%），在 Traffic 上几乎没变（+0.4%）。说明 CoIFNet 的去除模块变体实现对性能影响不大。

4. **两阶段方法仍然最优**：整体来看，Interp+iTrans（MSE 0.2834）和 SAITS+iTrans（0.2851）仍然优于所有原仓库对齐缺失感知方法。最好的原仓库对齐方法 MissTSM(含 skip connection)（0.3366）和 CoIFNet(含 TSBlock/AttentionBlock/RevON)（0.3473）仍落后 19-22%。
...
| 数据集 | 缺失类型 | 缺失率 | Interp+iTrans | MissTSM(含 skip connection) | CoIFNet(含 TSBlock/AttentionBlock/RevON) |
|---|---|---:|---:|---:|---:|
...
> **注**：Traffic+CRIB(原仓库对齐) 已跳过，不纳入高缺失率对比。Weather+CRIB(原仓库对齐) 在组 B 中有结果（随机点 50%: 0.1804, 70%: 0.2055; 连续片段 50%: 0.2118, 70%: 0.2290），均劣于 CoIFNet(含 TSBlock/AttentionBlock/RevON)，此处省略以保持表格一致。
...
1. **CoIFNet(含 TSBlock/AttentionBlock/RevON) 在 Weather 高缺失率下全面胜出**：在 Weather 的所有 4 个高缺失率配置中，CoIFNet(含 TSBlock/AttentionBlock/RevON) 均取得最优 MSE（0.1702~0.1850），显著优于 Interp+iTrans（0.1755~0.2260）。**这是第一次缺失感知方法在特定条件下明确超越两阶段方法**，改善幅度 2.7%~18.1%。

2. **MissTSM(含 skip connection) 在 Traffic 70% 连续片段缺失下反超**：在最极端的配置（Traffic, 连续片段, 70%缺失）下，MissTSM(含 skip connection)（0.5930）优于 Interp+iTrans（0.6529），改善 9.2%。这支持了"存在缺失率阈值"的假设。

3. **Interp+iTrans 在高缺失率下退化明显**：尤其在连续片段缺失中，Weather 从 0.1876（30%）升至 0.2260（70%），退化 20.5%。而 CoIFNet(含 TSBlock/AttentionBlock/RevON) 从 0.1656（30%）仅升至 0.1850（70%），退化 11.7%，鲁棒性更强。

4. **CRIB(原仓库对齐) 在高缺失率下表现不佳**：Weather 上 CRIB(原仓库对齐) 在所有高缺失率配置中均劣于 CoIFNet(含 TSBlock/AttentionBlock/RevON) 和 Interp+iTrans。Traffic+CRIB(原仓库对齐) 已跳过。

5. **存在"缺失率阈值"效应**：在 Weather 上，约 30~50% 缺失率是方法排名反转的分界点——低于此阈值，简单方法足够好；高于此阈值，CoIFNet(含 TSBlock/AttentionBlock/RevON) 的联合建模优势开始体现。

---

## 四、组 C：SAITS+PatchTST 溢出补跑

### 4.1 结果

第一轮 SAITS+PatchTST 在 Weather/Electricity 上有 102 条溢出。本轮增加训练预算（20 epoch, patience=5）后，48 条全部成功完成，无溢出。

![SAITS+PatchTST 补跑结果](../figures_r2/fig5_saits_fix.png)
...
补跑后的 SAITS+PatchTST 在 Electricity 上表现优异（h96 MSE 0.1481~0.1640），与 SAITS+iTransformer 和 MissTSM(含 skip connection) 处于同一水平。Weather 上的表现也稳定（h96 MSE 0.1643~0.1709）。增加训练预算有效解决了数值稳定性问题。
...
| Weather | B: MissTSM(含 skip connection)（掩码参与注意力） | 0.1778 | 0.1853 |
| Weather | C: CRIB(原仓库对齐)（掩码+KL正则） | 0.1805 | 0.2118 |
| Traffic | A: Interp+iTrans（无掩码） | 0.4912 | 0.5389 |
| Traffic | B: MissTSM(含 skip connection)（掩码参与注意力） | 0.5743 | 0.5742 |
| Traffic | C: CRIB(原仓库对齐)（掩码+KL正则） | - | - |

### 5.2 组 D 发现

**与第一轮结论出现分化**：

- **Weather 上掩码有效**：MissTSM(含 skip connection)（0.1778/0.1853）优于 Interp+iTrans（0.1876/0.2009），在 50% 缺失率下改善 7.8%。这与第一轮"掩码无收益"的结论相反，说明原仓库对齐版本 MissTSM 的 skip connection 能有效利用掩码信息。

- **Traffic 上掩码仍无效**：MissTSM(含 skip connection)（0.5743）仍劣于 Interp+iTrans（0.4912）。高维数据集上掩码信息的利用仍然是一个未解决的问题。
...
| 数据集 | 缺失类型 | 预测长度 | Interp+iTrans | MissTSM(含 skip connection) | CRIB(原仓库对齐) | CoIFNet(含 TSBlock/AttentionBlock/RevON) |
|---|---|---:|---:|---:|---:|---:|
...
|---|---|---:|---:|---:|
| MissTSM(含 skip connection) | Weather | 10% | 0.6340 | 0.1742 |
| MissTSM(含 skip connection) | Weather | 30% | 0.6328 | 0.1700 |
| MissTSM(含 skip connection) | Electricity | 10% | 1.0038 | **0.1502** |
| MissTSM(含 skip connection) | Electricity | 30% | 1.0038 | **0.1517** |
| CRIB(原仓库对齐) | Weather | 10% | 0.6340 | 0.1801 |
| CRIB(原仓库对齐) | Weather | 30% | 0.6328 | 0.1781 |
| CRIB(原仓库对齐) | Electricity | 10% | 1.0038 | 0.1536 |
| CRIB(原仓库对齐) | Electricity | 30% | 1.0038 | 0.1599 |
| CoIFNet(含 TSBlock/AttentionBlock/RevON) | Weather | 10% | 0.1970 | **0.1675** |
| CoIFNet(含 TSBlock/AttentionBlock/RevON) | Weather | 30% | 0.1687 | 0.1695 |
| CoIFNet(含 TSBlock/AttentionBlock/RevON) | Electricity | 10% | 1.5454 | 0.1697 |
| CoIFNet(含 TSBlock/AttentionBlock/RevON) | Electricity | 30% | 1.9034 | 0.1711 |

### 7.2 组 F 发现

- CoIFNet(含 TSBlock/AttentionBlock/RevON) 的补值 MSE 最高（Electricity 上达 1.5~1.9），但预测 MSE 与 MissTSM(含 skip connection) 接近。**进一步印证了"补值好 ≠ 预测好"的结论**。
- MissTSM(含 skip connection) 在 Electricity 上的预测 MSE 最低（0.1502），但其补值 MSE 为 1.0，远非最优补值方法。
...
基于组 A 数据。由于 Traffic+CRIB(原仓库对齐) 已跳过，CRIB(原仓库对齐) 仅在 Weather+Electricity 上有结果，与其他方法不可直接比较，因此分开报告。
...
| 3 | MissTSM(含 skip connection) (R2) | 0.3366 | +18.8% |
| 4 | CoIFNet(含 TSBlock/AttentionBlock/RevON) (R2) | 0.3473 | +22.5% |

**仅 Weather + Electricity（含 CRIB(原仓库对齐) 的可比范围）**：

| 排名 | 方法 | 平均 MSE |
|---:|---|---:|
| 1 | MissTSM(含 skip connection) | 0.2035 |
| 2 | CoIFNet(含 TSBlock/AttentionBlock/RevON) | 0.2038 |
| 3 | CRIB(原仓库对齐) | 0.2498 |

> CRIB(原仓库对齐) 在 Weather+Electricity 上排名末位，但差距远小于之前混入不完整 Traffic 数据时的表现（0.4591）。
...
| F2: 预测模型选择 > 缺失处理策略 | **维持**：原仓库对齐缺失感知方法仍未改变这一结论 | 无修正 |
| F3: 缺失感知方法全面落后 | **部分修正**：MissTSM(含 skip connection) 在 Electricity 上反超两阶段基准；CoIFNet(含 TSBlock/AttentionBlock/RevON) 在 Weather 高缺失率下反超 | 条件性修正 |
| F4: 补值好 ≠ 预测好 | **维持**：组 F 进一步验证 | 无修正 |
| F5: 加掩码反而损害性能 | **部分修正**：原仓库对齐版本 MissTSM 在 Weather 上掩码有正效，但 Traffic 上仍无效 | 条件性修正 |

### 8.3 实验假设验证

| 预期场景 | 是否出现 | 说明 |
|---|---|---|
| 场景一：原仓库对齐版本提升但仍不及两阶段 | **MissTSM/CoIFNet: 是** | MissTSM 在 Electricity 上提升 11.6%，在 Traffic 上提升 18.3%，但全局仍不及两阶段 |
| 场景二：高缺失率下反超 | **CoIFNet: 是（Weather）** | CoIFNet(含 TSBlock/AttentionBlock/RevON) 在 Weather 50%/70% 缺失下全面胜出，存在"缺失率阈值" |
| | **MissTSM: 是（Traffic 70%）** | Traffic 连续片段 70% 下 MissTSM 反超 Interp+iTrans |
| 场景三：原仓库对齐版本全面胜出 | **否** | 全面胜出未发生；CRIB(原仓库对齐) 反而大幅退化 |

### 8.4 意外发现

1. **CRIB(原仓库对齐) 是本轮最大的"负面惊喜"**：原仓库对齐版本引入的 patching + MultivariateNormal + 双视图机制在 Electricity 上导致退化 39%（h336 MSE 0.53，R1 去除模块变体为 0.19），可能是 MultivariateNormal 在高维空间中优化不稳定。此外 Traffic+CRIB(原仓库对齐) 因计算成本过高（每 epoch 504 秒，batch_size 只能压到 4）而全部跳过，也侧面说明该方法在高维数据集上不具可行性。

2. **MissTSM 的 skip connection 是关键改进**：原仓库对齐版本 MissTSM 相比去除模块变体的核心差异是 `x = mask * x_inp + (1-mask) * out`（保留观测值），这一简单机制在 Electricity 上贡献了 11.6% 的改善。

3. **"缺失率阈值"假设得到部分验证**：在 Weather 上，约 30~50% 的缺失率是 CoIFNet(含 TSBlock/AttentionBlock/RevON) 从"不如两阶段"到"优于两阶段"的分界点。但这个阈值效应未在 Traffic 上复现（CoIFNet(含 TSBlock/AttentionBlock/RevON) 在 Traffic 上始终落后）。
...
1. **原仓库对齐实现改善了 MissTSM（+11~18%）和 CoIFNet（+0~5%），但 CRIB 在 Electricity 上退化 39%，Traffic 上因计算代价过高跳过**。模型模块差异并非第一轮"缺失感知方法落后"的主要原因。

2. **两阶段方法（Interp+iTrans）在全局平均上仍然最优**（MSE 0.2834），优于最好的原仓库对齐缺失感知方法 MissTSM(含 skip connection)（0.3366，+18.8%）。

3. **存在条件性反转**：
   - MissTSM(含 skip connection) 在 Electricity（321 通道）上反超两阶段方法
   - CoIFNet(含 TSBlock/AttentionBlock/RevON) 在 Weather + 高缺失率（50%/70%）下反超两阶段方法
   - MissTSM(含 skip connection) 在 Traffic + 极端缺失（70% 连续片段）下反超
...
### 10.2 为什么 CRIB(原仓库对齐) 在 Electricity 上严重退化？
...
没有 skip 时（第一轮去除模块变体），**所有位置**的值都被 MissTSMLayer 的输出替换——包括那些本来就是正确观测值的位置。MissTSMLayer 的跨变量注意力（`misstsm.py:105`）将每个时间步的所有变量信息聚合为一个向量，再投影回 C 维。这个过程必然引入误差：即使某个变量有观测值，它也要经过 "嵌入 → 注意力聚合 → 投影" 这条路径重建，而非直接使用原始值。
...
### 10.4 为什么 CoIFNet(含 TSBlock/AttentionBlock/RevON) 在 Weather 高缺失率下反超两阶段？
...
### 10.6 为什么 CoIFNet 去除模块变体 vs 原仓库对齐版本差异很小？

2.2 节数据显示 CoIFNet 的改善幅度仅 -1.2%~+5%。原仓库对齐版本的主要变更是：
...
核心原因是：**CoIFNet 去除模块变体已经保留了其最关键的结构创新——联合补值-预测的双输出框架和 mask 拼接输入**（`coifnet.py:301`）。原仓库对齐版本增加的组件都是"锦上添花"级别的改进，不改变联合建模的基本有效性。
...
| 文件 | 内容 |
|---|---|
| `fig1_main_comparison.png` | 组 A 主对比：原仓库对齐版本 vs 去除模块变体 vs 两阶段基准 |
| `fig2_high_missing_rate.png` | 组 B 高缺失率探测（50%/70%） |
| `fig3_r1_vs_r2.png` | R1 去除模块变体 vs R2 原仓库对齐版本柱状图 |
| `fig4_missing_rate_trend.png` | 缺失率全景趋势（10% → 70%） |
| `fig5_saits_fix.png` | 组 C SAITS+PatchTST 溢出补跑结果 |
| `fig6_mask_ablation.png` | 组 D 掩码消融重验证 |
| `fig7_extended.png` | 组 E 扩展场景（变量通道/混合缺失） |
| `fig8_error_prop.png` | 组 F 补值误差 vs 预测误差 |
| `fig9_improvement_heatmap.png` | 去除模块变体→原仓库对齐版本改善幅度热力图 |
